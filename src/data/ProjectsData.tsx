...
export const projectsData: ProjectData[] = [
  {
    ...
    description: [
      "This academic project, developed for my bachelor’s thesis in Tomsk State University Russia, aims to simplify navigation for travelers passing through Soekarno-Hatta Airport. Airports can often be confusing, so the app provides step-by-step guidance with a zoomable map and directional lines. Users can select their starting point and destination, or simply pick a location to see its position on the map.",
  ...
  },
  {
    ...
    description: [
      ...
      "Built with Flutter and Dart, the app leverages GetX for state management and follows an MVC architecture for scalability. Firebase Firestore powers the real-time database, ensuring instant updates for inventory and store status—a critical feature for reliability. The project was a hands-on lesson in solving tangible problems with user-centric design and efficient technical execution."
    ],
    image: "/eb-27-lamp-edouard-wilfrid-buquet.jpg",
    ...
    >
      ...
      <MorphingDialogContainer>
        <MorphingDialogContent
          ...
          className='pointer-events-auto relative flex mx-5 sm:mx-20 lg:mx-70 h-[90vh] w-full flex-col overflow-hidden bg-gray-300'
        >
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            />
            <div className='p-6'>
              <MorphingDialogTitle className='text-2xl font-bold text-black mb-3 mx-2'>
          {title}
              </MorphingDialogTitle>
              <MorphingDialogSubtitle className='font-semibold text-zinc-600 mx-2'>
          {subtitle}
              </MorphingDialogSubtitle>
          <MorphingDialogDescription
          ...
          {description.map((paragraph, index) => (
            <p key={index} className='mt-3 text-gray-500 text-justify mx-2'>
            {paragraph}
            </p>
          ))}
          {link && linkHeader &&(
            <a
            className='mx-2 mt-5 inline-flex text-black hover:text-[#88ccb4] transition-colors group'
            href={link}
            target='_blank'
